...
//        }
    ...
    function _add_replace_ecotax($action, $object, $user, $langs, $conf)
    {
        ...
        if ($object->special_code == 2 && in_array($action, array('LINEORDER_DELETE', 'LINEPROPAL_DELETE', 'LINEBILL_DELETE', 'LINEORDER_SUPPLIER_DELETE', 'LINEBILL_SUPPLIER_DELETE','LINEBILL_SUPPLIER_DELETE','LINEORDER_SUPPLIER_DELETE','LINEBILL_SUPPLIER_DELETE'))) return 0;
        ...
        if ($object->element == 'commande_fournisseurdet' ||  get_class($object) == 'CommandeFournisseurLigne'){
            $fieldparentid = 'fk_commande';
            $parentobject = new CommandeFournisseur($this->db);
        }
//        if ($object->element == "facture_fourn_det" || get_class($object) == "SupplierInvoiceLine"){
//            $fieldparentid = "fk_facture";
//            $parentobject = new FactureFournisseur($this->db);
//        }
        ...
        foreach ($lines as $key => $line) {
            ...
            if ($line->special_code == 2 || $line->fk_product == $conf->global->WEEE_PRODUCT_ID) {// This line is an already existing service line ecotax
            ...
            if (is_object($tmpecotaxline[$ecocateg]) && $idlineecotax[$ecocateg] > 0) {    // If ecotax line already exists for ecocateg
                if ($ecoamount[$ecocateg]) {
                    ...
                    if ((float)DOL_VERSION < 7.0) {
                        ...
                        if ($parentobject->table_element == 'commande_fournisseur') $result = $tmpecotaxline[$ecocateg]->update(0);
//                        if ($parentobject->table_element == 'facture_fourn') $result = $tmpecotaxline[$ecocateg]->update(0);
                    } else {
                        if ($parentobject->table_element == 'facture') $result = $tmpecotaxline[$ecocateg]->update($user, 0);
                        ...
                        if ($parentobject->table_element == 'commande_fournisseur') $result = $tmpecotaxline[$ecocateg]->update($user,0);
//                        if ($parentobject->table_element == 'facture_fourn') $result = $tmpecotaxline[$ecocateg]->update($user,0);
                    }
                    ...
                    if ($result > 0) {
                        ...
                        if ($parentobject->table_element == 'commande_fournisseurdet') $sql = 'UPDATE ' . MAIN_DB_PREFIX . $parentobject->table_element_line . ' SET buy_price_ht = ' . $tmpecotaxline[$ecocateg]->buy_price_ht . ' WHERE rowid = ' . $tmpecotaxline[$ecocateg]->id;
                        ...
                        if ($parentobject->table_element == 'commande_fournisseur')  $sql = 'UPDATE ' . MAIN_DB_PREFIX . $parentobject->table_element_line . ' SET total_ht = ' . $tmpecotaxline[$ecocateg]->buy_price_ht . ' WHERE rowid = ' . $tmpecotaxline[$ecocateg]->id;
//                        if ($parentobject->table_element == 'facture_fourn')  $sql = 'UPDATE ' . MAIN_DB_PREFIX . $parentobject->table_element_line . ' SET total_ht = ' . $tmpecotaxline[$ecocateg]->buy_price_ht . ' WHERE rowid = ' . $tmpecotaxline[$ecocateg]->id;
                        if ($sql) {
            ...
            } else // If ecotax line does not yet exists for ecocateg and we need it
            {
                ...
                if ($parentobject->table_element == "commande_fournisseur") $result = $parentobject->addline($desc,$ecoamount[$ecocateg],1,$txtva,0,0,$product_id,0,'',0.0,'HT',$ecoamount[$ecocateg],0,0,1,null,null,0,0,'','');
                ...
                //if ($parentobject->table_element == "facture_fourn") $result = $parentobject->addline($desc,$ecoamount[$ecocateg],$txtva,0,0,1,$product_id,0,null,null,0,'','HT',0,-1,1,0,null,0,0,'',$special_code);
